...
describe('createHookRegistry', () => {
  it('registers and emits hooks in order', async () => {
    ...
    registry.register('beforeParse', () => { order.push(1) })
    registry.register('beforeParse', () => { order.push(2) })
    registry.register('beforeParse', () => { order.push(3) })
  ...
  it('emits to correct hook name only', async () => {
    ...
    registry.register('beforeParse', () => { order.push('beforeParse') })
    registry.register('afterParse', () => { order.push('afterParse') })
...
describe('onGlobalFlags short-circuit', () => {
  ...
  it('does not short-circuit when shortCircuit() is not called', async () => {
    ...
    registry.register('onGlobalFlags', () => { order.push('first') })
    registry.register('onGlobalFlags', () => { order.push('second') })
...
describe('cleanup hook', () => {
  it('always runs all handlers even if one throws', async () => {
    ...
    registry.register('cleanup', () => { order.push('first') })
    registry.register('cleanup', () => {
      order.push('second')
      throw new Error('cleanup failed')
    })
    registry.register('cleanup', () => { order.push('third') })
