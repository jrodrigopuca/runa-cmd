...
import type { PluginConfig, CommandTree, Middleware, OptionMeta, HookName, HookHandler } from '../types.js'
...
describe('runPluginSetup', () => {
  it('calls setup in order', async () => {
    const order: string[] = []
    const a = makePlugin('a', { setup: () => { order.push('a') } })
    const b = makePlugin('b', { setup: () => { order.push('b') } })
...
describe('runPluginCleanup', () => {
  it('calls cleanup in reverse order', async () => {
    const order: string[] = []
    const a = makePlugin('a', { cleanup: () => { order.push('a') } })
    const b = makePlugin('b', { cleanup: () => { order.push('b') } })
    const c = makePlugin('c', { cleanup: () => { order.push('c') } })
